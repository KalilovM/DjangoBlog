...
class PostAdmin(admin.ModelAdmin):
    ...
    viewers_count.short_description = "Количество просмотров"
...
class CommentAdmin(MPTTModelAdmin):
    ...
    def save_model(
        ...
        if (
            parent
            and Comment.objects.filter(id=parent, post_id=post, is_active=True).exists()
        ):
            ...
            messages.error(
                request, "Родительский комментарий принадлежит другому посту."
            )
        else:
            super().save_model(request, obj, form, change)


to_register = [(Post, PostAdmin), (Comment, CommentAdmin)]
